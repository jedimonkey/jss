 ...
 */
type ErrorHandlingPagesQueryResult = { site: { siteInfo: { errorHandling: ErrorHandlingPagesType } } };
...
export class GraphQLErrorHandlingPagesService {
  ...
  async fetchErrorHandling(): Promise<ErrorHandlingPagesType | null> {
    ...
    return (<Promise<ErrorHandlingPagesQueryResult>>this.graphQLClient.request(
      this.query,
      {
        siteName,
        language,
    ...
    .then((result: ErrorHandlingPagesQueryResult) => result.site.siteInfo ? result.site.siteInfo.errorHandling : null)
